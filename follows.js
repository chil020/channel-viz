(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'hq9P4hMbRusiOmUHSnUGWlqOs1gnlYcWoQlTBlb4DybTQrLH', // Unique master Xively API key to be used as a default
		defaultFeeds	= [994534883], // Comma separated array of Xively Feed ID numbers
		applicationName	= 'SOUTHERN SENSORS', // Replaces Xively logo in the header
			...
			xively.feed.history(id, {  duration: "6hours", interval: 30 }, function (data) {
				if(data.id == id) {
					...
					$('#feed-' + data.id + ' .duration-week').click(function() {
						$('#loadingData').foundation('reveal', 'open');
						updateFeeds(data.id, thisFeedDatastreams, '1week', 30);
						return false;
					});
